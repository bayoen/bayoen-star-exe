...
- 초기화는 '**메인 > Menu > Reset**'입니다. 모든 수치를 초기화 합니다. 만약 게임 중이라면, 현재 별은 초기화 되지 않습니다.

- 세부설정은 '**메인 > Menu > Settings**'입니다. 여러가지 기능을 추가하고 있습니다. 각 항목에 마우스를 올리면 한국어 설명이 있습니다.

- 모드변경은 '**메인 > Menu > Mode**'입니다. 모드는 **Star**(현재 별), **Star+**(누적 별), **Game**(게임/왕관)의 조합으로 구성되어 있어 편한 걸로 선택하시면 됩니다.
